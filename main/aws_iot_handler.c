...
void process_aws_iot_command( MQTTContext_t * pMqttContext, const char *command , int command_length){
    ...
    if(!memcmp(command, GET_INFO , GET_INFO_LEN)){
        memcpy(temp_response , AWS_GET_INFO_RESPONSE , AWS_GET_INFO_RESPONSE_LEN);
        if(xSemaphoreTake(adc_data_mutex, pdMS_TO_TICKS(5000)) == pdTRUE){
            publishToTopic(pMqttContext , MQTT_RESPONSE_TOPIC , temp_response, MQTTQoS1); 
            xSemaphoreGive(adc_data_mutex); 
        }        
        free(temp_response);
        return;
    }
    else if(!memcmp(command, "off1" , 4)){
        gpio_set_level(GPIO_OUTPUT_1 , 0);
        memcpy(temp_response , "rf1" , 3);
    }
    else if(!memcmp(command, "on1" , 3)){
        gpio_set_level(GPIO_OUTPUT_1 , 1);
        memcpy(temp_response , "rn1" , 3);
    }
    else if(!memcmp(command, "off2" , 4)){
        gpio_set_level(GPIO_OUTPUT_2 , 0);
        memcpy(temp_response , "rf2" , 3);
    }
    else if(!memcmp(command, "on2" , 3)){
        gpio_set_level(GPIO_OUTPUT_2 , 1);
        memcpy(temp_response , "rn2" , 3);
    }
    else if(!memcmp(command, "off3" , 4)){
        gpio_set_level(GPIO_OUTPUT_3 , 0);
        memcpy(temp_response , "rf3" , 3);
    }
    else if(!memcmp(command, "on3" , 3)){
        gpio_set_level(GPIO_OUTPUT_3 , 1);
        memcpy(temp_response , "rn3" , 3);
    }
    else if(!memcmp(command, "off4" , 4)){
        gpio_set_level(GPIO_OUTPUT_4 , 0);
        memcpy(temp_response , "rf4" , 3);
    }
    else if(!memcmp(command, "on4" , 3)){
        gpio_set_level(GPIO_OUTPUT_4 , 1);
        memcpy(temp_response , "rn4" , 3);
    }
    else{
    ...
    if(is_command_valid){
        ...
        if(xSemaphoreTake(adc_data_mutex, pdMS_TO_TICKS(5000)) == pdTRUE){
            publishToTopic(pMqttContext , MQTT_RESPONSE_TOPIC , temp_response2, MQTTQoS1); 
            xSemaphoreGive(adc_data_mutex); 
        }          
    }
#ifdef DB_AWS_IOT
...
int publishToTopic( MQTTContext_t * pMqttContext , char *msg_topic , char *message , MQTTQoS_t mqttQos )
{
    ...
    else
    {
        ...
        outgoingPublishPackets[ publishIndex ].pubInfo.qos = mqttQos;//MQTTQoS1;
        ...
        if(send_mqtt_message){
            ...
            if(xSemaphoreTake(adc_data_mutex, 0) == pdTRUE){
                for(int i = 0 ; i < mqtt_response_counter ; i++){
                    publishToTopic(pMqttContext , MQTT_RESPONSE_TOPIC , mqtt_response_list[i].data , mqtt_response_list[i].mqttQos);
                }
