...
void process_tcp_data(char* rx_buffer , int rx_buffer_len , int sock){
    ...
    if(is_data_valid){
        if(xSemaphoreTake(adc_data_mutex, pdMS_TO_TICKS(5000)) == pdTRUE){
            send_data_to_clients(SEND_TO_ALL, temp_response, 3, MQTTQoS1 , true , true);
